...
from classifiers.build import build_torch_dataset_for_mol, build_dataloader
from compent import Global_Var
from compent.utils import move_to_device
from dataset import build_dataset
from pseudo_label.metric import cal_metric_binary_class
...
class Evaler_MOL():
    def __init__(self, cfg):
        ...
        for item_key in self.eval_on:
            ...
            if (item_key == 'train'):
                try:
                    item_dataset = Global_Var.get('dataset')
                except Exception as e:
                    print(str(e))
                    item_dataset = build_dataset(item_cfg)
            else:
                item_dataset = build_dataset(item_cfg)
            dataset_to_eval[item_key] = item_dataset
        self.split_to_loader = {}
        for item_key in self.eval_on:
            item_set = dataset_to_eval[item_key]
            torch_dataset = build_torch_dataset_for_mol(cfg, item_set.smiles, item_set.labels)
            torch_loader = build_dataloader(cfg, torch_dataset, for_train = False)
            self.split_to_loader[item_key] = torch_loader
        # self.thr = cfg.eval_thr
        # if (cfg.classifier.output_dim == 1):
        ...
        with torch.no_grad():
            for item_split in self.eval_on:
                ...
                for batch in cur_loader:
                    ...
                    batch_smiles = batch['batch_smiles']
                    all_smiles += batch_smiles
                ...
                metric = cal_metric_binary_class(all_pred_scores = all_pred_score,
                                                 all_gt_labels = all_gt_labels, thr = 0.5)
                self.logger.info(metric)
                results[item_split] = metric

                if (item_split == 'train' and return_labeled_samples):
                    # all_pred_classes = all_pred_score > self.thr
                    all_pred_classes = all_pred_classes.long().tolist()
                    results['train_smiles'] = all_smiles
