...
from classifiers.build import CLASSIFIER
...
class GIN(nn.Module):
    ...
    def __init__(self, num_node_emb_list, num_edge_emb_list,
                 num_layers = 5, emb_dim = 300, JK = 'last', dropout = 0., readout = "mean", output_dim = 2):
        ...
        self.linear_fc = nn.Linear(emb_dim, output_dim)
    ...
    def forward(self, input, return_last_feature = False, for_pretrain = False):
        ...
        graph_feats_origin = self.readout(input, final_node_feats)
        graph_feats = F.relu(graph_feats_origin)
        graph_feats = self.linear_fc(graph_feats)
        if (return_last_feature == False and for_pretrain == False):
            return graph_feats
        elif (return_last_feature):
            return graph_feats, graph_feats_origin
        elif (for_pretrain):
            return graph_feats, final_node_feats
            # graph_feats: [1,3]
            # final_node_feats [number_nodes,512]
