...
from classifiers.build import build_dataloader, build_torch_dataset_for_mol, build_classifier
from classifiers.evaler.eval_mol import Evaler_MOL
...
class Trainer_ST_MOL():
    ...
    def build_train_loader(self, smiles, labels):
        training_dataset = build_torch_dataset_for_mol(self.cfg, smiles = smiles, labels = labels, for_train = True)
    ...
    def train_model(self, smiles, labels):
        ITR = Global_Var.get('ITR')
        self.ITR = ITR
        ...
        while itr_self_training < self.cfg.max_ST_itr:
            ...
            smiles, labels, val_auc = self.__do_train__(smiles, labels, itr_self_training)
            if (val_auc > last_best_val):
                last_best_val = val_auc
                itr_self_training += 1
            else:
    ...
    def __do_train__(self, smiles, labels, itr_ST):
        self.logger.info('starting self training:{}'.format(itr_ST))
        train_dataloader = self.build_train_loader(smiles = smiles, labels = labels)
