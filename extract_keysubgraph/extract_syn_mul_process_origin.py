...
def cal_occurrences_times(item_subgraph, big_graph_list):
    print('len big_graph_list:{}'.format(len(big_graph_list)))
    count = 0
    for index, item_big in enumerate(big_graph_list):
        print('big node number:{} sub number:{}'.format(item_big.number_of_nodes(), item_subgraph.number_of_nodes()))
        if (judge_contain_a_subgraph(big_graph = item_big, subgraph = item_subgraph)):
            count += 1
        # print('index:{} count:{}'.format(index, count))
    return count
...
def one_process(rank, class_index, queue_input, global_result_list, big_graph_each_class):
    ...
    while not queue_input.empty():
        ...
        print('cal subgraph:{}, rank:{}'.format(item_subgraph, rank))
...
#     extractor = Extract_Syn_Multi_Process()
...
#
#     extractor(mol_dataset.graphs_networkx, mol_dataset.labels, model)
