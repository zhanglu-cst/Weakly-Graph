...
class Extract_Syn():
    def __init__(self, min_node_number = 4, max_node_number = 15, number_classes = 3, keep_subgraph_each_class = 10):
        ...
        self.top_K_mi_selected_for_final = 100
    ...
    def __call__(self, graphs, labels, model):
        ...
        if os.path.exists('subgraph_to_avg_mi.pkl'):
            ...
            subgraphs_mi_each_classes = self.cal_MI_each_classes(graphs, labels, model)
        ...
        for class_index, subgraph_to_avg_mi_cur_class in enumerate(subgraph_to_avg_mi):
            ...
            for item_subgraph, item_avgmi in list_subgraph_avgmi_cur_class:
                ...
                item_TF = self.cal_TF(count_time = cur_subgraph_count_all_class[class_index],
                                      instance_number_cur_class = len(big_graph_each_class[class_index]))
                item_IDF = self.cal_IDF(class_index, cur_subgraph_count_all_class, big_graph_each_class)
        ...
        res = []
        for list_subgraph_score_cur_class in list_subgraph_score_each_class:
            list_subgraph_score_cur_class = list_subgraph_score_cur_class[:self.keep_subgraph_each_class]
            res.append(list_subgraph_score_cur_class)
...
if __name__ == '__main__':
    ...
    checkpoint = CheckPointer('/remote-home/zhanglu/weakly_molecular/work_dir/syn_small/small_version_supervised/',
                              rank = 0)
    model = model.cuda()
    model.eval()
