...
DIR_SAVE = '/remote-home/zhanglu/weakly_molecular/data/syn_large_v1'
...
def generate_key_subgraph_k_regular(d_list = (4, 5, 6), n_list = (5, 6, 7, 8, 9, 10)):
    ans = []
    for d in d_list:
        for n in n_list:
            if (n * d % 2 == 0 and n > d):
...
def generate_key_subgraph_k_star(n_list = (7, 8, 9, 10)):
    ans = []
    for n in n_list:
...
all_key_subgraphs = [keywords_class1, keywords_class2]
...
def pick_random_subgraph():
    class_indexes = random.sample(population = [0, 1], k = 1)
    all_subgraphs = []
...
while True:
    ...
    n = get_random_value(mu = total_number_nodes + 20, sigma = 6, lower_bound = total_number_nodes,
                         upper_bound = total_number_nodes + 25)
    n = int(n)
