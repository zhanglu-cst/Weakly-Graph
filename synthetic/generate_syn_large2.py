...
from compent.networkx_ops import judge_contain_a_subgraph, Graphs_Set, get_random_graph
from compent.utils import get_random_value

NUM_GRAPHS = 2000

DIR_SAVE = '/remote-home/zhanglu/weakly_molecular/data/syn_large_v2'
...
def generate_key_subgraph_k_regular(d_list = (5, 6), n_list = (6, 7, 8, 9)):
    ans = []
    for d in d_list:
        for n in n_list:
            if (n * d % 2 == 0 and n > d):
                ...
                # show_a_graph(item)
                ans.append(item)
    print('number key regular:{}'.format(len(ans)))
    return ans
...
def generate_key_subgraph_k_star(n_list = (5, 6, 7, 8, 9, 10)):
    ...
    for n in n_list:
        ...
        # show_a_graph(item)
        ans.append(item)
    print('number k star:{}'.format(len(ans)))
    return ans


def generate_key_subgraph_grid(w = (2, 3,), h = (3, 4, 5), upper_bound = 15):
    ans = []
    for item_w in w:
        for item_h in h:
            if (item_w * item_h <= upper_bound):
                item = networkx.grid_graph(dim = (item_w, item_h))
                # show_a_graph(item)
                ans.append(item)
    print('number grid:{}'.format(len(ans)))
    return ans


def generate_key_subgraph_addtional():
    ans = []
    # m_list = [1, 2, 3]
    # n_list = [1, 2, 3]
    # for item_m in m_list:
    #     for item_n in n_list:
    #         item = networkx.triangular_lattice_graph(item_m, item_n)
    #         ans.append(item)
    n_list = [7, 11, 13]
    for item_n in n_list:
        item = networkx.cycle_graph(item_n)
        ans.append(item)
    print('number:{}'.format(len(ans)))
    # for n in [2, 3]:
    #     for m in [3, 4]:
    #         item = networkx.ring_of_cliques(n, m)
    #         ans.append(item)
    return ans
...
keywords_class4 = generate_key_subgraph_addtional()
all_key_subgraphs = [keywords_class1, keywords_class2, keywords_class3, keywords_class4]
...
def pick_random_subgraph():
    class_indexes = random.sample(population = [0, 1, 2, 3], k = 1)
    all_subgraphs = []
...
while True:
    ...
    n = get_random_value(mu = total_number_nodes + 17, sigma = 6, lower_bound = total_number_nodes,
                         upper_bound = total_number_nodes + 22)
    n = int(n)
    print('total number nodes of graph:{}'.format(n))
    p = get_random_value(mu = 0.2, sigma = 0.2, lower_bound = 0.1, upper_bound = 0.5)
    ...
    static_label[GT_label] += 1
    print('static_label:{}'.format(static_label))
    print('-----------------\n\n\n')
...
# ---------------- key subgraph ---------------
path_keygraph = os.path.join(DIR_SAVE, 'key_subgraph_each_class_GTs.pkl')
with open(path_keygraph, 'wb') as f:
    pickle.dump(all_key_subgraphs, f)

all_key_subgraphs_used = []
for item_class_keys in all_key_subgraphs:
    all_key_subgraphs_used.append([item_class_keys[-1]])
path_keygraph = os.path.join(DIR_SAVE, 'key_subgraph_each_class.pkl')
with open(path_keygraph, 'wb') as f:
    pickle.dump(all_key_subgraphs_used, f)
